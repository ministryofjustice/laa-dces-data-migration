...
for CSV_FILE in "$CSV_DIR"$FILE_PATTERN*.csv; do
    ...
    # Write SQL statements to the SQL file
    {
        ...
        echo -e "    RAISE NOTICE 'Copying data...';"
        echo -e "END \$\$;"
        echo -n $BS
        echo -e $COPY_SQL
    } >> "$SQL_FILE"
    ...
    # Extract the row count from the footer
        ...
        # Add a check for the row count after creating the table and copying the data
        {
            ...
            echo -e "    SELECT COUNT(batch_id) INTO v_rowcount FROM $TABLE_NAME where json_footer is null;"
            ...
            echo -e "        RAISE NOTICE 'SUCCESS: Rowcount match: created entry in file_audit_table';"
            echo -e "    ELSE"
            echo -e "        $DROP_TABLE_SQL"
            echo -e "        RAISE NOTICE 'ERROR: Deleting $TABLE_NAME - Row count mismatch:=> Expected: %, Found: %', $FOOTER_ROWCOUNT, v_rowcount;"
            echo -e "    END IF;"
            echo -e "END \$\$;"
        } >> "$SQL_FILE"
