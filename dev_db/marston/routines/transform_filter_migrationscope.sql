...
-- It aggregates maximum loaded dates from `laacasenotes_20250106` and `laacasehistory_20250106`, and selects relevant `caseid`, `maatid`, and `lacescaseid` values from `laacasedetails_20250106`.
...
BEGIN
    ...
    WITH 
        -- Aggregate maximum loadedon dates from laacasenotes_20250106
        max_casenotes AS (
            SELECT 
                caseid, 
    ...
    FROM marston.laacasedetails_20250106 a
    LEFT JOIN max_casenotes mn ON a.caseid = mn.caseid
    LEFT JOIN max_casehistory mh ON a.caseid = mh.caseid
    LEFT JOIN marston.laalacesdatawarehouse_20250106 dwh ON dwh.maatid = a.clientcasereference
