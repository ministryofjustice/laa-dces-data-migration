...
BEGIN
    ...
    SELECT 
        ...
        CASE
            ...
            WHEN ((aca.numberofcapitalassets = CASE WHEN la.numberofavailablecapitalassets ~ '^\d+$' THEN la.numberofavailablecapitalassets::integer ELSE 0 END) AND (CAST(NULLIF(la.totalavailablecapitalassets, 'NULL') AS numeric) = aca.total_cap_amt) OR (aca.caseid IS NULL OR aca.total_cap_amt = 0)) THEN 'N'
